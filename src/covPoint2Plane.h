...
Eigen::MatrixXd computeICPCovPoint2Plane(std::vector<Eigen::Vector3d>& data_pi, std::vector<Eigen::Vector3d>& model_qi, std::vector<Eigen::Vector3d>& model_qi_normals, double sensor_noise_cov, Eigen::Matrix4d& transform)
{
    ...
    /****  Calculating d2J_dX2  ****/
    for (int s = 0; s < data_pi.size(); ++s )
    {
        double pix = data_pi[s](0);
        double piy = data_pi[s](1);
        double piz = data_pi[s](2);
        double qix = model_qi[s](0);
        double qiy = model_qi[s](1);
        double qiz = model_qi[s](2);

        double nix = model_qi_normals[s](0);
        double niy = model_qi_normals[s](1);
        double niz = model_qi_normals[s](2);
  ...
  */
    ...
    for (int k = 0; k < n ; ++k) // row
    {
        ...
        double pix = data_pi[k](0);
        double piy = data_pi[k](1);
        double piz = data_pi[k](2);
        double qix = model_qi[k](0);
        double qiy = model_qi[k](1);
        double qiz = model_qi[k](2);

        double nix = model_qi_normals[k](0);
        double niy = model_qi_normals[k](1);
        double niz = model_qi_normals[k](2);
    ...
    cov_z = sensor_noise_cov * Eigen::MatrixXd::Identity(6*n,6*n);
